...
Based on the HBT experiment, a simplified explanation of Incoherent Diffractive Imaging (IDI) can easily be made for thermal light sources in a classical view  by application of the Siegert relation.
...
\end{equation}
the difference of the intensity-intensity correlation from unity is proportional (with a contrast determining constant $\beta^2$) to the Fourier transform of the arrangement of emitters with $q=\vec{k_1}-\vec{k_2}$ (see \fref{fig:scatteringvectors}).
