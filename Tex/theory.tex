...
The application of the Wiener Khinchin theorem then gives $g^1$

On the other hand, for a Lorentzian light source, $<g_1(\tau)>=e^{-|\tau/| \tau_c}$, showing the coherence time  $\tau_c$ with the definition above is the time, after which $g^1$ has decreased to $e^{-1} g^1(0)$.
...
\end{equation}
If the distances from $r$ to the two slits are (approximately) equal, $R_{1}\approx R_{2} \approx R$, the intensity at $\vec{r}$ is
\begin{align*}
	...
	+\left[\left\langle E^{*}\left(\vec{r}_{1}, t\right) E\left(\vec{r}_{2}, t\right)\right\rangle e^{i\left(\vec{k}_{1}-\vec{k}_{2}\right) \cdot \vec{r}+i \varphi_{0}}+\text{c.c}\right] \\
	\propto 1+V\left|\gamma_{12}\right| \cos \left[\left(\vec{k}_{1}-\vec{k}_{2}\right) \cdot \vec{r}+\Delta \varphi\right]
\end{align*}
...
If the sources $A$ and $B$ produce spherical electromagnetic waves $E_{A,B}=c_{A,B} e^{i k\left|\vec{r}-\vec{r}_{1,2}\right|+i \phi_{A,B}} /\left|\vec{r}-\vec{r}_{A,B}\right|$ with $\phi_{A,B}$ random but constant phases and ignoring polarisation, the total intensity at detector 1 $I_{1}$ (and similiar $I_{2}$ at detector 2) is
\begin{equation}
	I_{1} =
	\frac{1}{L^{2}}\left(
	|c_A|^{2}
...
Visibility of the speckle pattern
...
For thermal light with Gaussian statistics, Isserlis' theorem 
...
This is the basic principle used in IDI to recover spatial information from the intensity correlation.
\begin{figure}
	\centering
