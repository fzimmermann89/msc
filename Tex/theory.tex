...
\begin{equation}
	\vec{E}(\vec{r},t)=\vec{E}_0(k,t) \frac{e^{i\vec{r}\vec{k}-iwt}}{\left|\vec{r}\right|} \,,
\end{equation}
...
\end{equation} 
Now \fref{eq:interference} can be written as $I=I_{1}+I_{2}+2 \sqrt{I_{1} I_{2}} \Re\left[\gamma(\tau)\right]$ and (again for equal intensities) the visibility is $V(\tau)=\left|\gamma(\tau)\right|$ \cite{zernike1938,loudon2000}. As the Wiener-Khinchin theorem connects the power spectrum $F(w)$ with $\gamma$,
\begin{equation}
...
The \textit{coherence time }shall describe at which delay the visibility falls to approximately zero. One possible definition of the coherence time by Mandel based on the complex degree of coherence,
\begin{equation}
...
$\left<ABCD\right>=\left<AB\right>\left<CD\right>+\left<AC\right>\left<BD\right>+\left<AD\right>\left<BC\right>$ can be used to factorize \fref{eq:g2}, leading  independently of the spatial configuration of the sources to
\begin{equation}
...
\label{eq:modes}
\end{equation}
where $P(t)$ is he integration window weighting function and $K(t)$ its autocorrelation \cite{goodman2007}. If the integration windows is set by the integration time $T$ of the detector, $P(t)$ is a rectangular function with the width $T$ and
\begin{align*}
