...
\begin{figure}[!htb]
	...
	\centering
	\smallskip
	\includegraphics[width=0.9\linewidth]{images/pairs.png}
	\caption{Correlation pairs dependence on q} 
\end{subfigure}\\
	\end{tabular}
	\caption[Accessible reciprocal space]{The accesible reciprocal space with an 1024x1024\,pixel (50\,um) pixelsize at XXX distance and XXX\,keV is shown in a) and b). In a), the surface of the Ewald sphere accessible in an diffraction experiment is shown in green for comparison. IDI allows a reconstruction of a three dimensional volume. The position of Bragg peaks for an XXX is shown in b). As the accessible $q_z$ is depended on $q_x$/$q_y$ and smaller the larger the latter two, in this setup a precise alignment of the detector with regard to the lattice planes if useful to be able to image the maximal number of peaks. Using a square, centered, planar detector with uniform pixelsize, the number of correlation pairs for resulting in the same $\vec{q}$ depends on $\vec{q}$ as shown in c).}
\end{figure}
...
\begin{figure}
	...
	\begin{subfigure}[b]{0.4\textwidth}
		\includegraphics[width=\linewidth]{images/modes.pdf}
