...
For each local maxima over the noise threshold XXX a check is performed if by fusing with a single neighbouring pixel 

 afterwards, as well as for high photon counts, the signal is discretized by only counting events with an energy consistent with the sum of integer multples of the florescence energy and the excitation energy  within a window of XXX \,ev.
