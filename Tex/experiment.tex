...
\begin{equation}
	w (x)= (1-p)\delta(x)+\rect\left(\frac{x-1/2}{E_{signal}}\right)*p
\end{equation}
is performed, thus smearing the peaks to lower energies with a strength parameter $p$ (initialized as $10^{-4}$).
...
The mean number of signal and scattering photons as well as the peak width and the known charge sharing PSF for the detectors is used to find the thresholds for a classification of the pixel values into signal photon numbers as described in \fref{sec:chargesharing}.
