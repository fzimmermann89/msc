...
class datasetreader:
    ...
    def __init__(self, datasets, file = None, sizecache=20, willread=None):            
        ...
        self._readerprocess = [
            _mp.Process(
                name=f'reader_process_{i}', target=self._reader, 
                args=(self._rindices[i], self._rcache[i], shapecache, self._lastread, self._readevents[i], self._writeevents[i], filenames[i], ds_names[i], willread)
            )
            for i in range(nds)
        ]
        for p in self._readerprocess: p.start()
        ...
        with _h5py.File(filename, 'r') as file:
            ...
            while True:
                ...
                if nempty == 0:
                    ...
                    readevent.clear()
                    continue
                #print(_mp.current_process().name,'writing',indices,indices[empty])
                if readall:
                    ...
                    if inextwrite == readindices.shape[0]:
                        return
                #print(_mp.current_process().name,'written',indices,indices[empty])
    ...
    def __getitem__(self, i):
        ...
        if self._lastread.value > n:
            raise IndexError('must request images monotonicly increasing')
        ...
        for ct in r:
            ...
            while True:
                ...
                if len(index) == 1:
                    ...
                    break
                #print('waiting for',n,ct,'at',self._indices[ct])
