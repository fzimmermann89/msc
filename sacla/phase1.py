...
#from idi.util.h5util import *
from accum import * # remove dependence on idi package
from h5util import *
...
parser.add_argument('--assembled', dest='assembled', action='store_true', help='use assembled detectors')
...
if args.assembled:
    dets = {
        'dual': detectorinfo_t(True, True, ['detector_2d_assembled_1']),
        'octal': detectorinfo_t(True, True, ['detector_2d_assembled_2']),
    }
else:
    dets = {
        'dual': detectorinfo_t(True, True, ['detector_2d_2', 'detector_2d_3']),
        'octal': detectorinfo_t(
            True,
            True,
            [
                'detector_2d_4',
...
def attenuator_cal(pulses):
    '''convert attenuator motor values to thickness'''
...
def calcedgeintensity(d):
    '''intensity at the detector edges'''
    s0, s1 = d.shape
    edges = (
...
# create output file and load old data
...
for fi, f in enumerate(args.inputfiles):
        ...
        for tilenr, detname in enumerate(detinfo.detectors):
            ...
            for i in range(len(det)):
                ...
                if detinfo.savemeta:
                    ...
                    hists[i, :] = fast_histogram.histogram1d(
                        np.clip(ev[np.abs(ev) > 1e-3], histrange[0] - histres, histrange[1] + histres),
                    ...
                    if kalpha != 0:
                        accums[f'detectors/{detoutname}/tile{tilenr}/singlephotoncount'] += ((ev > (kalpha - 1000)) & (ev < (kalpha + 1000))).astype(
                            float
                        )
                    if i % 10 == 0:
                ...
                if detinfo.savedata:
                    ...
                    appenddata(
                        outfile, f'detectors/{detoutname}/tile{tilenr}/data', np.clip(ev, -(2 ** 31), (2 ** 31) - 1).astype(np.int32)[None, ...]
                    )
            if detinfo.savemeta:
            ...
            if detinfo.savemeta or detinfo.savedata:
                ...
                attr['detector_rotation_steps'] = np.array(
                    (int(np.rint(det.detector_rotation_angle_in_degree / 90)), (det.detector_rotation_angle_in_degree + 45) % 90 - 45)
                )
                attr['detector_tile_position_in_pixels'] = np.array(
                    (-1, 1) * np.array(det.detector_coordinate_in_micro_meter)[1::-1] / np.array(det.pixel_size_in_micro_meter)
                )
    ...
    try:  # appending point data
        ...
        appenddata(
            outfile,
            'attenuator/attenuator_eh_5_Si_thickness_in_meter',
            np.array(attenuator_cal(inputfile.attenuator_eh_5_Si_uncalibrated), dtype=np.float32),
        )
